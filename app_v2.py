...
# --- 頁面設定 ---
st.set_page_config(page_title="內部人籌碼雷達 (V10 介面優化版)", layout="wide")
...
if data_res and data_res[0] is not None:
    ...
    # --- Tabs ---
    tab1, tab2, tab3, tab4 = st.tabs(["📊 個股分析", "🏆 相關性排行", "🚀 連續買超掃描", "🛠️ 技術面掃描 (MACD)"])
    ...
    with tab1:
        ...
        if stock_id in rank_df.index:
            ...
            cols[1].metric(f"大戶(>{selected_tier_key}) 連動", f"{val_corr:.2f}", delta="High" if val_corr > 0.8 else None)
            ...
            fig.update_layout(title="籌碼趨勢 (週線)", height=350, hovermode="x unified", margin=dict(b=0))
            ...
            if 'raw_price' in raw_data and stock_id in raw_data['raw_price'].columns:
                ...
                if not daily_series.empty:
                    ...
                    if not daily_series.empty:
                        ...
                        fig2.update_layout(height=400, xaxis_rangeslider_visible=False, template="plotly_dark", 
                                           title=f"技術指標 (日線)", margin=dict(t=30))
                        st.plotly_chart(fig2, use_container_width=True)

            # 2. 詳細籌碼表
            st.subheader("近 12 週數據")
